	...
	function editDispatchRecode(PID) {
		...
		if (PID == 0) {//新增
			...
			index = layer.open({
				...
				anim : 5,
				area : [ '95%', '95%' ],
				content : '${ctx}/baseInfo/editWorkOnRecord?driverID='
						+ $("#driverID").val() + "&ID=" + ID,
				end : function(index) {
		...
		if (PID == 2) {
			...
			var confirm = layer.confirm('确认要删除该培训记录吗？', {
				offset: '200px',
				btn : [ '确定', '取消' ]
			}, function() {
				layer.close(confirm);
				...
				$.ajax({
					type : "POST",
					url : "${ctx}/baseInfo/deleteDriverTrainningRecord",
					data : data,
